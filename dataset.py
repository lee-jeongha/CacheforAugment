...
class ImageFolderWithCache(torchvision.datasets.DatasetFolder):
    def __init__(
        ...
        self.idx_to_be_dismissed = []   # [ index ]
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        """
        if index in self.cache_sample.keys():
    ...
    def _cache(self, idx, sample, target, reuse_factor=0, loss=0):
        ...
        self.cache_sample[idx] = [ sample.numpy(), np.int64(target), reuse_factor, abs(loss) ]
    ...
    def cache_from_idx(self, idx: int, sample, target, loss):
        if idx in self.cache_sample.keys():
            pass

        if -abs(loss) < self.max_loss_candidates:
            return

        elif idx in self.idx_to_be_dismissed:
            # TODO: if already in `idx_to_be_dismissed` -> update?
            pass

        elif idx not in self.cache_sample.keys():
            # replace
            try:
                ...
                return
            #self.idx_to_be_dismissed.append(idx)
    ...
    def cache_from_idx_first_epoch(self, idx: int, sample, target, loss):
        ...
        '''
        if len(self.cache_sample) == 0:
            self._cache(idx, sample, target, 0, loss)
            
            self.evict_candidates_heap.append((-abs(loss), idx))
            #self.idx_to_be_dismissed.append(idx)
            self.max_loss_candidates = -abs(loss)

        elif len(self.cache_sample) < self.cache_len:
            # insert
            self._cache(idx, sample, target, 0, loss)
    ...
    def release_from_idx(self, idx: int, has_to_delete_idx=False):
        ...
        #if idx in self.idx_to_be_dismissed: 
        if has_to_delete_idx:
            try:
                self.idx_to_be_dismissed.remove(idx)
            except ValueError:  # Some elements that have been replaced in this epoch may not be in the `self.idx_to_be_dismissed`.
                pass
        return reuse_factor